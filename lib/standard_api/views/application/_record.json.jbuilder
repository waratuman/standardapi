  ...
  when ActiveRecord::Reflection::HasManyReflection, ActiveRecord::Reflection::HasAndBelongsToManyReflection, ActiveRecord::Reflection::ThroughReflection
    ...
    json.cache_if!(can_cache, can_cache ? association_cache_key(record, inc, subinc) : nil) do
      ...
      json.set! inc do
        ...
        sub_records = sub_records.limit(subinc.delete('limit')) if subinc['limit']
        sub_records = sub_records.offset(subinc.delete('offset')) if subinc['offset']
        sub_records = sub_records.order(subinc.delete('order')) if subinc['order']
        sub_records = sub_records.filter(subinc.delete('where')) if subinc['where']
        sub_records = sub_records.distinct if subinc['distinct']
        sub_records = sub_records.distinct_on(subinc.delete('distinct_on')) if subinc['distinct_on']
